  ...
  <div>
    ...
    <div class="mt-5 bg-white shadow overflow-hidden sm:rounded-md">
      <ul role="list" class="divide-y divide-gray-200">
        <li v-for="(barcode, index) in products" :key="index" @click="openQR(barcode)">
          <modal :name="'qr-modal-' + barcode" :classes="['qr-modal']">
              <div class="qr-container">
                <qrcode-vue :value="`${origin}/#/products/${barcode}/show`" :size="200" level="H" />
                <p>{{$route.query.page}}</p>
                <router-link :to="`/products/${barcode}/show`" class="block hover:bg-gray-50">Click</router-link>
              </div>
          </modal>
...
export default {
  ...
  methods: {
    openQR(barcode) {
      this.$modal.show(`qr-modal-${barcode}`);
    },
  },
